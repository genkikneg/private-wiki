...
@section('content')
@php
    $editingBugId = old('editing_bug_report_id');
    $createTitle = $editingBugId ? '' : old('title');
    $createDescription = $editingBugId ? '' : old('description');
@endphp
<div class="max-w-4xl">
        ...
        <form method="POST" action="{{ route('bug-timeline.store') }}" class="space-y-4">
            ...
            <div>
                ...
                <textarea id="description" 
                          ...
                          required>{{ $createDescription }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
    ...
    <div class="space-y-6">
        @forelse($bugReports as $bugReport)
            @php
                $isEditing = $editingBugId && (string) $editingBugId === (string) $bugReport->id;
                $editTitle = $isEditing ? old('title') : $bugReport->title;
                $editDescription = $isEditing ? old('description') : $bugReport->description;
            @endphp
            <div class="bg-white rounded-lg shadow-sm p-6"
                 x-data="{ showEditModal: {{ $isEditing ? 'true' : 'false' }}, showDeleteConfirm: false }"
                 x-on:keydown.escape.window="showEditModal = false; showDeleteConfirm = false;">
                <div class="flex justify-between items-start mb-4">
                    ...
                    <div class="flex space-x-2">
                        <button type="button"
                                ...
                                class="text-sm px-3 py-1 rounded border border-blue-600 text-blue-600 hover:bg-blue-50">
                            編集
                        </button>
                ...
                <div x-show="showEditModal"
                    ...
                    <div @click.away="showEditModal = false"
                         ...
                         class="bg-white rounded-lg p-6 max-w-lg mx-4 w-full shadow-xl">
                        <h3 class="text-lg font-semibold text-gray-900 mb-4">バグレポートの編集</h3>

                        <form method="POST" action="{{ route('bug-timeline.update', $bugReport) }}" class="space-y-4">
                            @csrf
                            @method('PATCH')
                            <input type="hidden" name="editing_bug_report_id" value="{{ $bugReport->id }}">

                            <div>
                                <label for="edit-title-{{ $bugReport->id }}" class="block text-sm font-medium text-gray-700 mb-1">
                                    タイトル <span class="text-red-500">*</span>
                                </label>
                                <input type="text"
                                       id="edit-title-{{ $bugReport->id }}"
                                       name="title"
                                       value="{{ $editTitle }}"
                                       class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                       required>
                                @if ($isEditing)
                                    @error('title', 'updateBugReport')
                                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                                    @enderror
                                @endif
                            </div>
                            ...
                            <div>
                                <label for="edit-description-{{ $bugReport->id }}" class="block text-sm font-medium text-gray-700 mb-1">
                                    詳細 <span class="text-red-500">*</span>
                                </label>
                                <textarea id="edit-description-{{ $bugReport->id }}"
                                          name="description"
                                          rows="4"
                                          class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                          required>{{ $editDescription }}</textarea>
                                @if ($isEditing)
                                    @error('description', 'updateBugReport')
                                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                                    @enderror
                                @endif
                            </div>
